(ns ddbj.main
  "Kotodama WASM entrypoint for the Ddbj clean-room actor (L4) — Clojure port.
  ...
  facts (`ddbj.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Patient"          :plural "patients"          :id-prefix "ddbj_pat"
    :fields [:identifier :givenName :familyName :birthDate] :required [:identifier :givenName]
    :coerce {}                       :refs {}}
   {:entity "Observation"      :plural "observations"      :id-prefix "ddbj_obs"
    :fields [:patientId :code :value :unit]                :required [:code :value]
    :coerce {}                       :refs {:patientId "Patient"}}
   {:entity "Condition"        :plural "conditions"        :id-prefix "ddbj_con"
    :fields [:patientId :code :clinicalStatus]            :required [:code :clinicalStatus]
    :coerce {}                       :refs {:patientId "Patient"}}
   {:entity "DiagnosticReport" :plural "diagnosticreports" :id-prefix "ddbj_dia"
    :fields [:patientId :code :status :issuedAt]          :required [:code :status]
    :coerce {}                       :refs {:patientId "Patient"}}
   {:entity "Sequence"         :plural "sequences"         :id-prefix "ddbj_seq"
    :fields [:patientId :accession :referenceGenome]      :required [:accession :referenceGenome]
    :coerce {}                       :refs {:patientId "Patient"}}
   {:entity "Specimen"         :plural "specimens"         :id-prefix "ddbj_spe"
    :fields [:patientId :type :collectedAt]               :required [:type :collectedAt]
    :coerce {}                       :refs {:patientId "Patient"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ddbj.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ddbj-compat" :tier tier :entities entities} 200])
